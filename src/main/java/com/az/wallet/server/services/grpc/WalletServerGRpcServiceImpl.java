package com.az.wallet.server.services.grpc;
...
import com.az.wallet.server.services.UserService;
import com.az.wallet.server.services.UserWalletService;
import com.az.wallet.server.util.CurrencyUtils;
...
public class WalletServerGRpcServiceImpl extends WalletServerServiceGrpc.WalletServerServiceImplBase {
    ...
    */
    @Override
    public void balance(BalanceRequest balanceRequest, StreamObserver<BalanceResponse> responseObserver) {
        log.info("Server received request for Balance!");
        int userId = balanceRequest.getUserId();
        if(userId>0) {
    ...
    */
    private void checkUserWalletBalance(int userId,StreamObserver<BalanceResponse> responseObserver){
        ...
        if(!CollectionUtils.isEmpty(userWalletEntityList)) {
            ...
            BalanceResponse balanceResponse = BalanceResponse.newBuilder().addAllWalletBalance(walletBalanceList).build();
